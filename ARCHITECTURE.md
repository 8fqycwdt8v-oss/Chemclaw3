...
| --- | --- | --- |
| `core/` | — | The shared kernel: config, database, HTTP, ids, logging, errors, embeddings, the Temporal client, the one way to attach a database this system does *not* own (`connect.py` — resolve a `module:callable` driver, read its `*_env` credentials, used by ingest, publish and the vector-store registry alike), the process metrics registry, the one answer to "is this path the checkout this process runs from?" (`checkout.py` — asked by `kg/git_writer.py`, which refuses to commit notes into the running application's tree, and by `core/netguard.py`, which therefore does not derive that tree's git remote into the egress allowlist; two spellings of it disagreed once and the disagreement was a widening), the bounded-LRU (`bounded.py`, the one eviction policy the R3.2 consolidation gave the four call sites that used to hand-roll it — the front door's session/budget/rate-limiter state and the agent's attachment store), and the ambient-turn primitives every layer stamps or reads (identity, session id, turn signals, the capability-tool registry). Everything imports it; it has zero module-scope import of a sibling, and exactly one declared *lazy* (function-scope) exception — `core/logging.py`'s redaction filter, which resolves connector bearer-token env-var names by importing `connectors.registry` inside a function so the value can be redacted from logs. `tests/test_layering.py` enforces both halves. **The one file here that is neither Python nor a package `README.md` is not an exception to "`src/` is all the code" but an instance of it**: `core/netguard_preload.c` is the compiled half of the egress guard, an `LD_PRELOAD` interposition on libc that refuses what `core/netguard.py` measurably cannot reach — grpc's C-core and Temporal's Rust sdk-core — so it lives beside the Python module it extends rather than in a directory of its own (`D-2026-09-12-the-layer-that-binds-grpc-is-libc-not-socket-py`). |
...
| `connectors/` | 2 + 3 | The capability seam. One bundle per capability (`chem`, `calc`, `bo`, `results`, `safety`, `molfp`, `rxnfp`, `rxnpredict`), each colocating its `connector.yaml` manifest, its Temporal worker, its own `skills/`, and — for the ones this release still runs — its MCP tool server. Adding a capability is adding a directory here — no core edit. **A bundle with an `endpoint:` and no `server/` is a declaration we do not run**: `chem`'s, `safety`'s and `rxnpredict`'s capabilities are `Chemclaw3-mcp`'s, and what stays here is the manifest four validators resolve tool names through — plus, for `safety`, its `skills/` tree, because judgment is layer 3 and does not follow the engine — and the chart's `connectors.<name>.url` saying where to dial it (D-2026-08-09). `results` has neither, and that is the other shape: a jobs-only bundle whose whole surface is a durable job, so the chart renders it a Temporal worker and no server at all. |
...
| `hypotheses/` | — | Competing explanations, ranked. Pure computation behind `durable/hypothesis_tournament.py`: the Elo-scale rating fit and its intervals, Swiss pairing, the two mechanical screen rules, and the rendering. No Temporal, no LangGraph, no model client — the same engine/orchestration split `science/bo` has against the `bo` bundle, and it sits here rather than under `science/` because nothing in it is chemistry: a Bradley-Terry fit over judged comparisons is infrastructure that would read identically for any domain. **What ranks is not what removes**: only `screen.py` deletes a candidate, and a model critic's objections enter the tournament as evidence, per `D-2026-08-16-a-second-judge-is-a-second-answer-about-the-same-answer`. |
...
| `analytical/` | — | The prescriptive tier for **results**, beside `protocols/`'s tier for what to run: acceptance criteria, and the deterministic verdict a set of measurements earns against them. Four verdicts rather than a boolean, because a criterion nothing measured and a result the limit cannot be compared with are both "no answer" and every check that returns a pass/fail spells both of them *pass*. No judgment and no instrument model — it reads numbers a person measured and answers from `core/units` arithmetic, which is also where `Measurement.compare` finally gets the production caller its own docstring described. `stability.py` is the same tier over time: where a trending attribute's 95% bound meets that criterion, bounded by what ICH Q1E permits extrapolating, and never called a shelf life. |
| `templates/` | — | Step templates: the manifest, registry and resolver. |
| `evals/` | — | The eval harness and metrics. |
| `operations/` | — | The operational read model: what this system *did*, read back out of the record it already keeps — tool use by outcome, durable-job runs, what the agent wrote into the graph, what was changed outside this deployment, and turn-level spend. Five tables — `audit_events`, `job_records`, `plan_approvals`, `turn_costs`, `effects` — that had readers but no way to be read *across* (the package's own `__init__.py` names them, and says why the earlier "writers and no reader" framing was false for four of them); `PostgresAuditSink` still exposes only `record` and `flush`. Counts and identifiers only, never a caller's free text, because one shared corpus means an aggregate is visible to everyone who can reach the agent. Writes nothing. |
...
| `data/` | Every corpus the code reads at runtime, each behind a `CHEMCLAW_*` setting: `evals/` (the case-set, retrieval corpus and committed baseline), `templates/` (the shipped step templates), `profiles/` (agent profiles), `vendored/` (build-time datasets with their provenance), `eln-exports/` (sample ELN drops), `commitments/` (a programme's committed work, mirrored in). `data/README.md` holds the same list with each setting's name, and `tests/test_repo_map.py` checks both against the tree in both directions — this row read five for a while, and the missing sixth was the one whose *absence* is silent. |
...
One image, five process roles, chosen by `CHEMCLAW_COMPONENT` and dispatched by `deploy/entrypoint.sh`.
...
- [`Chemclaw3-mcp`](https://github.com/8fqycwdt8v-oss/Chemclaw3-mcp) — the MCP tool fleet: one
  capability per server, each with the `connector.yaml` this repo picks up with no code change.
...
design and is **historical** — right about the four layers, and knowing the connector seam that now
carries every tool, job and skill only through the notes retrofitted into it.

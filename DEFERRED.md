...
| Conversation-history trimming/summarization in the agent | Retrieval is already bounded (capped `gather_evidence`, sized excerpts, offline distillation into campaign notes), so a single answer is token-frugal; MAF owns the chat thread and long-conversation compaction | Real chat sessions grow long enough to overflow — then add a MAF thread-summarization/trim policy (not a new store) |

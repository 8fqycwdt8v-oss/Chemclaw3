...
- **turns driven / mock requests served**: 819 / 595
...
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 2 | 4 | 44 | 6.0 | 6.1 | 0.57 | 6.86 |
| 4 | 7 | 41 | 6.3 | 8.1 | 0.77 | 5.69 |
| 8 | 8 | 40 | 7.0 | 8.3 | 0.95 | 5.71 |
| 16 | 16 | 32 | 9.0 | 14.2 | 1.12 | 3.36 |
| 32 | 32 | 16 | 27.2 | 27.7 | 1.15 | 1.73 |
...
| D | a completed job is findable through find_past_jobs afterwards | PASS | 1 tool result(s); result[0]="[JobRecordSummary(job_id='calc-compute_reaction_energy-f2dff99a26b08b2" |
...
| H | an injection string is searched for, and audit_events is still there | PASS | audit_events 5191 → 5192; the search itself is the 1 new row(s) |
| H | an unparseable reaction SMILES does not kill the turn — the call came back | PASS | HTTP 200, answered=False, error=empty_answer, announced=1 returned=1, result[0]='chunks=[EvidenceChunk(content=\'<retrieved-note-3a1849ba573971c7 id="co' |
...
| A | the admission cap is load-bearing (goodput rises with it) | PASS | cap 2: 0.57 answered/s → cap 32: 1.15 answered/s |
| A | the sweep's own noise is small enough to read a knee against | PASS | largest within-cap spread 13% over 3 sample(s) per cap (ceiling 15%, minimum 2 sample(s)) |
| A | the sweep resolves the knee rather than running out of range | PASS | goodput stops improving at cap 16 (steps must beat the 13% noise floor over 3 samples) |
...
| E | a job survives its connector worker being SIGKILLed mid-flight | PASS | at kill: RUNNING, activity held by 26016@vm; after restart: COMPLETED 584s later (heartbeat timeout is 600s); job_records rows: 1 |
| E | the front door recovers from a Postgres restart without being restarted itself | PASS | postmaster start time '2026-08-28 22:50:59.148786+00:00' -> '2026-08-28 23:17:53.593534+00:00' (restarted); 23/24 in-flight turns survived the bounce; a fresh turn answered 2.0s after it |
...
| B | find_notes bodies ran | PASS | 1 audited call(s) during this run (3713 → 3714 lifetime) |
| B | gather_evidence bodies ran | PASS | 1 audited call(s) during this run (57 → 58 lifetime) |
| B | expand_note bodies ran | PASS | 1 audited call(s) during this run (29 → 30 lifetime) |
| M | every model call this run made was served by the mock | PASS | 595 mock request(s) served against 269 turn(s) driven |

**69/70 checks passed**, over the families that ran.

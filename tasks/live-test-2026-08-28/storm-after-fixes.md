...
- **turns driven / mock requests served**: 819 / 597
...
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 2 | 4 | 44 | 6.0 | 6.1 | 0.59 | 7.03 |
| 4 | 7 | 41 | 6.5 | 7.9 | 0.78 | 5.34 |
| 8 | 8 | 40 | 7.1 | 8.6 | 0.92 | 5.51 |
| 16 | 16 | 32 | 8.9 | 14.1 | 1.12 | 3.37 |
| 32 | 32 | 16 | 26.4 | 26.9 | 1.19 | 1.78 |
...
| D | a completed job is findable through find_past_jobs afterwards | PASS | 1 tool result(s); result[0]="[JobRecordSummary(job_id='calc-compute_reaction_energy-78297df668e2345" |
...
| H | an injection string is searched for, and audit_events is still there | PASS | audit_events 4203 → 4204; the search itself is the 1 new row(s) |
| H | an unparseable reaction SMILES does not kill the turn — the call came back | PASS | HTTP 200, answered=False, error=empty_answer, announced=1 returned=1, result[0]='chunks=[EvidenceChunk(content=\'<retrieved-note-d636527b502b35cc id="co' |
...
| A | the admission cap is load-bearing (goodput rises with it) | PASS | cap 2: 0.59 answered/s → cap 32: 1.19 answered/s |
| A | the sweep's own noise is small enough to read a knee against | **FAIL** | largest within-cap spread 19% over 3 sample(s) per cap (ceiling 15%, minimum 2 sample(s)) |
| A | the sweep resolves the knee rather than running out of range | **FAIL** | unreadable: the largest within-cap spread is 19%, above the 15% ceiling — one sample's error is wider than the steps being compared, so no knee is claimed. Raise --sweep-repeats |
| E | a disconnected session accepts a new turn without waiting out the lease | PASS | accepted after 11.3s (lease is 60.0s, the detached turn thinks for ~8s); status codes [409, 409, 409, 409] |
| E | a job survives its connector worker being SIGKILLed mid-flight | PASS | at kill: RUNNING, activity held by 15380@vm; after restart: COMPLETED 584s later (heartbeat timeout is 600s); job_records rows: 1 |
| E | the front door recovers from a Postgres restart without being restarted itself | PASS | postmaster start time '2026-08-28 21:27:20.187331+00:00' -> '2026-08-28 22:24:33.968004+00:00' (restarted); 23/24 in-flight turns survived the bounce; a fresh turn answered 2.0s after it |
...
| B | find_notes bodies ran | PASS | 1 audited call(s) during this run (2942 → 2943 lifetime) |
| B | gather_evidence bodies ran | PASS | 1 audited call(s) during this run (53 → 54 lifetime) |
| B | expand_note bodies ran | PASS | 1 audited call(s) during this run (27 → 28 lifetime) |
| M | every model call this run made was served by the mock | **FAIL** | 597 mock request(s) served against 819 turn(s) driven |

**65/70 checks passed**, over the families that ran.

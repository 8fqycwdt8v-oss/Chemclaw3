...
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 2 | 6 | 42 | 6.2 | 6.2 | 0.84 | 6.69 |
| 4 | 9 | 39 | 6.9 | 7.4 | 1.08 | 5.71 |
| 8 | 16 | 32 | 8.3 | 10.1 | 1.49 | 4.48 |
| 16 | 25 | 23 | 11.7 | 13.9 | 1.64 | 3.73 |
| 32 | 32 | 16 | 17.0 | 17.3 | 1.82 | 2.73 |
...
| D | a completed job is findable through find_past_jobs afterwards | PASS | 1 tool result(s); result[0]='[{"job_id": "calc-compute_reaction_energy-95bf654def856d98", "connecto' |
...
| D | the collision computed at most one result set | PASS | calculation_results 160 → 160 (one cold run writes ~3-6 rows) |
...
| H | an injection string is treated as a search string | PASS | audit_events 541 → 542 (a dropped table reads as 0) |
...
| A | the admission cap is load-bearing (goodput rises with it) | PASS | cap 2: 0.84 answered/s → cap 32: 1.82 answered/s |
| A | the sweep's own noise is small enough to read a knee against | **FAIL** | largest within-cap spread 15% over 3 sample(s) per cap |
| A | the sweep resolves the knee rather than running out of range | **FAIL** | no cap in (2, 4, 8, 16, 32) stops paying by more than the 15% noise floor — the sweep's top is a limit of the sweep, not of the system |
...
| B | find_notes bodies ran | PASS | 781 audited call(s) |
| B | gather_evidence bodies ran | PASS | 5 audited call(s) |
| B | expand_note bodies ran | PASS | 3 audited call(s) |

**29/31 checks passed**, over the families that ran.

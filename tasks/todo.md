...
this repo, thirteen qualify (`predict_pka`, `predict_solubility`, `compute_xtb_energy`,
...
- [ ] 1. `hypotheses/dispatch.py` — pure. `CheckCall` (tool + subject note id), the schema
...
- [ ] 2. `DiscriminatingCheck` gains `call: CheckCall | None`. A `computable` check without a
      resolvable call is reported as computable-but-refused, with the reason, rather than silently
      dropping to prose.
- [ ] 3. Ground the subject: resolve the note id against the corpus, require `type: compound`,
      require `compound_smiles` to pass `core.chem.require_canonical_smiles`. Refuse on each miss
      with a distinct reason.
- [ ] 4. `derive_check` asks for the structured call, and is shown the note ids the evidence sweep
      actually returned so the model selects from what the system saw rather than from memory.
- [ ] 5. `run_computable_check` loses its early return: open connector sessions the way
...
- [ ] 6. Verdict: a model reads the computed value against the check's stated expectation and
...
- [ ] 7. `report.py` renders a check that ran with its value, and a refused one with its reason.
- [ ] 8. Tests: the validator refuses a tool requiring a second argument, refuses an unresolvable
...
- [ ] 9. Delete the `BACKLOG.md` row in the same commit (the register's own rule), and write the
      ADR recording what the dispatcher does and does not cover.
- [ ] 10. `make lint type test` green; run the suite with the daemon up so the Postgres-backed

...
56. **A fixture that never varies is a test that never tests, and the axis it holds constant is the
    ...
    **Building the mechanism for that refuted its obvious form**, which is the transferable half:
    the proposal was to flag every `model_copy(update=…)` in a test whose model declares
    `extra="ignore"` or `"forbid"`. Instrumented — `model_copy` patched for a run, every copy
    re-validated and compared — it separates nothing: of the 149 sites the suite executes, none
    injects an undeclared key and none builds an object `model_validate` refuses, and `extra="ignore"`
    is pydantic's *default*, so the 80 sites declaring nothing behave exactly like the 6 declaring it.
    The danger is not a property of the object; it is the relation between fixture and assertion, and
    that is not readable from the call site. What is readable is what makes the relation possible, and
    `tests/test_model_copy_fixtures.py` derives it from the tree: the model's own `model_validator`
    bodies (a check the constructor runs and the copy skips) and `src/`'s own `model_validate(` calls
    (a boundary production really crosses). 27 fields over 22 of 151 sites, of which **two were weak
    and one was green over the deletion its own name implies.** So before building a flagging rule,
    instrument the thing and count how many of the sites it would flag are actually broken: a proxy
    that fires on four fifths of the tree is a proxy.

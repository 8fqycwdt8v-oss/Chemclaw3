...
`SourceType` registry layer is the "universal abstraction" `docs/planning/DEFERRED.md` reserves for the third
...
backdoor to the deliberately-unbuilt MAF graph-workflows, `docs/guides/harness-konzept.md §11`), *not* a
...
Each is an ADR-ready candidate `docs/planning/BACKLOG.md` item with a trigger and a rough size. Ordered by
...
- Use-case workflows: `agents/chemclaw_agent.py` (`build_agent`/`_INSTRUCTIONS`), `service/app.py:135`,
  `service/runner.py:46`, `docs/guides/harness-konzept.md §11`

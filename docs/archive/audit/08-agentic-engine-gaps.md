...
that is appropriate; the deferral of durable multi-step research is conscious (docs/planning/DEFERRED.md, "Durable
...
on the same key both computing — is consciously deferred as benign (docs/planning/DEFERRED.md, "Per-key in-flight
...
explicitly **deferred** (docs/planning/BACKLOG.md "MAF out-of-the-box features → Structured outputs").
...
tool-selection, prompt changes, or skill judgment. docs/planning/BACKLOG.md explicitly scopes this out ("the
...
| AG-13 | Evaluation harness | Chemistry-metric evals + tool-utility A/B + retrieval gold-set (D-056) | Automated agent-*behavior* / prompt / skill regression suite | Prompt/skill edits can silently regress tool-selection with no gate — weak for GxP reproducibility | Medium | L | **Deferred** — a faithful behavior eval needs the live internal LLM; a mock only tests the mock (`docs/planning/DEFERRED.md`) |
| AG-14 | Prompt/skill versioning & change mgmt | **Addressed (D-057):** every audit row now carries `deployment_revision`; migration `010` adds the column | Behavioral pre-live gate (that half is AG-13) | A past agent result now ties to the exact deployment revision that produced it | Medium | S |
| AG-15 | Concurrency / rate limits | **Addressed (D-057):** a config-capped `asyncio.Semaphore` admits at most `service_max_concurrent_turns` turns, shedding excess with 503 after an admission timeout | Per-*user* fairness (global cap only, for now) | Concurrent turns can no longer pile unbounded onto the shared endpoint; excess is shed, not amplified | Medium | M |

...
**Evidence:** No identity/authz check in any retrieval tool (confirmed: no `authorize`/`actor`/`identity` reference in `graph_tools.py`, `search_tools.py`, `research_tools.py`, `retrievers.py`). The authorization gate guards only *expensive triggers*, not reads (`agents/authz.py:22-42`); `skill_access.py` gates skill *visibility*, not document content. The Postgres-RLS graph mirror and per-project confidentiality are explicitly deferred (`docs/planning/DEFERRED.md:9`, `:61-63`): "Broad internal read access is fine for cross-project learning… Trigger: real, combinatorial project-level confidentiality requirements."
...
| KM-5 | Ranking/reranking | **Addressed (D-057):** `gather_evidence` ranks by `EvidenceChunk.score` before the cap — graph hits score by note `confidence`, structural hits by similarity; `confidence` is now wired | Finer lexical relevance for text hits (moot while the graph filter is whole-substring — ties to KM-4) | A broad sweep now drops the least-supported evidence first, not an arbitrary disk slice | Medium | M |
| KM-6 | Provenance & citation | **Addressed (D-055):** `NoteRef` now carries `created_by`/`source`/`confidence`/`valid_from`/`valid_to` | — (rich provenance surfaced at read) | Trace-to-origin met; the agent can now weigh a source without a second lookup | Low | S |
| KM-7 | Freshness & staleness | **Partly addressed (D-055):** `valid_to`/`valid_from` now enforced at read (discovery/report sweeps exclude non-current notes) | fp-index re-index on note *mutation* (still open) | Superseded/expired conditions no longer served as current fact; fp/graph drift on edit remains | Medium | M |
...
| KM-9 | Access control on retrieval | Absent — deferred by design (`docs/planning/DEFERRED.md:9`) | Per-caller doc/project scoping (RLS mirror) | Only matters when confidential+open projects share the graph | Low | L |
...
| KM-11 | Multi-modal / structured | Partial: reactions/molecules/tables first-class; analytical dropped (`docs/planning/DEFERRED.md:23`) | Spectra/chromatogram/image ingestion & linking | Structured chemistry retrievable; raw analytical proof deferred | Medium | L |
...
| KM-14 | Scale behavior | **Retrieval half addressed (D-057):** `load_notes` caches the parse behind a stat fingerprint that busts on any note change (so retrieval no longer re-parses the tree per query); O(n²) clustering stays deferred | Sub-quadratic clustering (still `docs/planning/DEFERRED.md`) | Interactive Q&A no longer re-parses the whole tree per query; the background clustering job's O(n²) is unchanged | Medium | M |
...
*Deferred-by-design, correctly (not counted as defects): access control on retrieval (KM-9, `docs/planning/DEFERRED.md:9` — gated on real project confidentiality), analytical multi-modal data (KM-11, `docs/planning/DEFERRED.md:23`), and the O(n²) clustering half of KM-14 (`docs/planning/DEFERRED.md:25`). Each has an ADR and a concrete revisit trigger.*

...
- The **live-infrastructure edges** remain as `docs/planning/DEFERRED.md`/`docs/planning/BACKLOG.md` describe (real Entra tenant,
...
- **ELN multi-ingest cursor guard** — `sync_eln_entries` now fails fast + non-retryably when more
  ...
  source would silently skip the lagging one). This is the interim validator `docs/planning/DEFERRED.md` names;
...
- **Critical deferred-items re-review** — every `docs/planning/DEFERRED.md` item re-checked against current
  reality; all remain correctly deferred except the cursor guard above. See the "Critical re-review"
  section in `docs/planning/DEFERRED.md`.
...
`docs/planning/DEFERRED.md` and `docs/planning/BACKLOG.md` for the itemized reasons.

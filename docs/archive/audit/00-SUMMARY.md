...
| KM-11 | Analytical multi-modal | Med (deferred) | Reactions/molecules/tables first-class; spectra/chromatograms dropped | Spectra/chromatogram/image ingestion & linking | Structured chemistry is retrievable; raw analytical proof is an **ADR-backed deferral** (`docs/planning/DEFERRED.md:23`), not a live gap | L |
...
| KM-9 | Access control on retrieval | Low (deferred) | Absent — **ADR-backed deferral** (`docs/planning/DEFERRED.md:9`) | Per-caller doc/project scoping (RLS mirror) | Only bites once confidential + open projects share one graph; the confidentiality boundary doesn't yet exist | L |
...
3. **COR-2 — Nextflow idempotency now or at the live edge.** The double-submit risk is real but the path
   ...
   Fix now (defensive) or defer with the rest of the HPC live edge (per `docs/planning/DEFERRED.md`)?

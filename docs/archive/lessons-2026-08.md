# Lessons, full text — 2026-07 to 2026-08

**The long form of every lesson `tasks/lessons.md` now carries as one rule.** Eighty sections,
written as they were learned, in the order they were learned. Nothing here is deleted or corrected;
where two sections say the same thing, both are kept, because *how many times* a lesson had to be
learned is itself the finding.

**Why it moved.** `CLAUDE.md` instructs a session to review this file at session start. It had
reached 1,937 lines across 80 sections — not readable at session start, so not read, so not
applied. The clearest evidence is in this file: **five separate sections record the same
`git checkout` mistake** (2026-07-31, 2026-08-01, 2026-08-05, 2026-08-11, 2026-08-12), about 103
lines for one lesson, and the fifth one says out loud what the shape means — *"The rule was already
written down and it did not fire, so the rule is the problem, not the memory."* A rule that is
1,937 lines from the top of a file does not fire.

`tasks/lessons.md` is now the digest: one short paragraph per distinct lesson, short enough to read
in full. Come here when a rule's paragraph is not enough and you want the incident that produced it
— the concrete failure, the measurement, and what was tried first.

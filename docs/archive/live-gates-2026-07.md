# The two authorization gates, run live (2026-07-31, D-157 / D-158)

*Closes DARK-1 and DARK-2 from `docs/planning/BACKLOG.md`. DARK-3 was already closed by D-153; see
the last section. The decisions are in `docs/decisions/D-157-…` and `D-158-…`; this is the record of
...
further defects — one in the D-157 fix itself, four in the path D-158 governs. This is the record
that matters, because each was invisible to a suite that had just passed.

### D-157: the approval outlived the request
...
### D-158: the template path had never executed

...
hinges on (§4.2, §5.1). Supersedes and completes `docs/archive/audit/10-config-extensibility.md`, whose
seven follow-ups are all shipped (`docs/planning/BACKLOG.md:63-99`) — this plan is the next architectural step,
...
execution engine: the LLM still chooses tool order, and `docs/guides/harness-konzept.md §11`'s deliberate
...
versioning (`docs/guides/workflow-versioning.md`), a resume story, and per-step authz. It is **specified
and not built**: the Rule-of-Three trigger is a second real use case that a profile provably cannot
express. Recorded in `docs/planning/BACKLOG.md` with that trigger, so it is a decision rather than an omission.
...
10. `.env.example`, Helm values/templates, `docs/guides/runbook.md`, ADRs in `docs/decisions/`.
...
Not built. Trigger recorded in `docs/planning/BACKLOG.md`.
...
1. **Entra auth modes are documented, not built.** The chosen option listed
   ...
   `docs/planning/BACKLOG.md:501` already blocks live Entra edges on a real tenant. Building either mode now would

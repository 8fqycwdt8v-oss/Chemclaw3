# D-166 — A template step runs as its requester, and four steps that had never run
...
4. …and the fix for (3) is to run the step **without the harness**, not to invent a session. A
   ...
   back the discretion the template was written to remove, and under D-165's gate would refuse every

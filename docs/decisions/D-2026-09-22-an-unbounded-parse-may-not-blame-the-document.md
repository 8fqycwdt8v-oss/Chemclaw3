...
- **`UnclassifiedParseError`**, a new `DocumentParseError` subclass, marks the two broad `except` arms in
  `parse_document`: a third-party parser failed and this system does not know why. That is a different
  fact from "we read it and it is over the limit", and only the first can be a memory failure wearing a
  parse error's clothes.
- **`read_without_a_ceiling(name, cause)`** is the wording that population earns on an unbounded path. It
  keeps the parser's own message verbatim, because an operator debugging a share needs it, and adds what
  the message cannot establish.
- A **classified** refusal passes through untouched — an unsupported format, an over-expanding archive,
  a scanned PDF. Those hold whether or not a ceiling was set, and burying them under a caveat about
  memory would be the same what-do-I-actually-know failure pointed the other way.
...
reject-and-continue net, the upload route, the isolate child. Verified that nothing in `src/` or `tests/`
...
in-process path and requires the caveat, drives a classified refusal through it and requires the caveat's
*absence*, and derives the set of broad arms from `parse.py`'s own AST — so a third broad arm added later
is either covered or red, rather than silently outside the type.

...
| [D-2026-08-27-the-gate-tells-the-truth-about-what-it-pushed](D-2026-08-27-the-gate-tells-the-truth-about-what-it-pushed.md) | The PR-gate's record could diverge from git four ways: a lease refreshed one line above the push (a plain force wearing the safe flag's name — a reviewer's fixup silently discarded), one error name for transient and structural failures (so no git fault ever retried), a per-version record under a per-note branch (two open rows, both marked merged by the webhook), and a no-push path that still recorded and counted. Now: gate commits carry a trailer and a foreign tip refuses; `GitRemoteError` is the retryable name; a new open version supersedes its predecessors (migration 057); `submit` returns what happened; dependencies never overwrite; and submissions to one remote serialize across pods on a Postgres advisory lock |
=======

...
AG-*/KM-*/`DEFERRED.md` entry, that is stated — restatement is deliberate only when this pass found
...
symbol outside tests returns the worker registration, `docs/parity-plan.md`, and `BACKLOG.md`.
...
consumer of the gated OCR/vision item in `docs/parity-plan.md` — which is currently gated on "a real
...
- The deferred "per-step species linking from free-text prose" is blocked *precisely* on this: its
  `DEFERRED.md` entry says linking needs "a name→SMILES tool," which does not exist.
...
`BACKLOG.md:568` has carried "chemical/biological safety layer — distinct from Entra-ID/RBAC" as an
...
`DEFERRED.md:14` defers external retrievers with the trigger *"after Phase 5b core; add as one more
...
`DEFERRED.md:30` defers compound notes until "compound notes exist (a later ELN step)" — a
...
decision (it has been open in `BACKLOG.md` since the research review) and should be taken before the
...
not read as contradicting `DEFERRED.md`:
...
- **Conversational multi-agent mesh** — a single agent plus role-scoped skills is the KISS answer, and
  the trigger in `docs/parity-plan.md` is a good one. Building it now would be the one-caller

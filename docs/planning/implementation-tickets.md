...
> D-043…D-047. Live edges (real tenant/broker) open. T1 front-door OIDC (`service/auth.py`); T3
...
> **Status: DONE (offline).** ADR **D-048** (+ D-A5a Tower REST). `workflows/hpc/nextflow.py`
...
> **Status: manifests DONE** (ADR **D-049**, D-A6/D-A6a). `deploy/`: one rootless multi-target image
...
> **Status: DONE.** ADR **D-050**. `sources/base.py` (`DataSource` composes the existing
...
- **Touch:** ~`docs/reference/architektur.md` §6 (OpenShift instead of Azure AI Foundry/Container Apps;
...
- **Touch:** ~`docs/decisions/` — append D-A1…D-A9 (one per CHECKMATE) and finalize D-020 (harness

...
| **Spectra and images** (multimodal analytical data) | Text/CSV/TSV and PDF/PPTX/DOCX/XLSX ingress all landed (AGT-3, D-089), each read through the format's own document model. What is left needs OCR/vision — which is why a *scanned* PDF is refused by name rather than guessed at. The gated item in `docs/planning/parity-plan.md` | OCR/vision is adopted |

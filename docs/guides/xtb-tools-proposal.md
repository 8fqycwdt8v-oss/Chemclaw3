...
(D-0xx) in `docs/decisions/`.
**Scope:** how ChemClaw should use the xTB ecosystem (tblite / xtb / CREST) and what tool surface
the agent should see.
**Non-scope:** DFT/HPC accuracy work (deferred, see `docs/planning/DEFERRED.md`), docking, ML potentials.
...
These come straight from `CLAUDE.md` and `docs/reference/architektur.md`; every decision below is justified
against them.
...
Recording these keeps the scope honest and gives the next reader the reasons, per `docs/planning/DEFERRED.md`
practice.
...
rationale in **D-095** (seams), **D-096** (BO featurization), **D-097** (geometry policy +
skill catalogue) and **D-098** (X3/X4). Four things this document got wrong or under-called:

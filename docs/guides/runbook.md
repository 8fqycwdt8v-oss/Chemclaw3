...
the gap the worker's probes leave, and it is narrower than this section used to say: `/readyz` is
deliberately not a liveness signal, because restarting on a lost broker connection would turn an
ordinary reconnect into a crash loop — but since 2026-09-04 it *does* report one, because

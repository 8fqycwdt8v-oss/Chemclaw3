...
| 3.4 | explain why it suggests these conditions (exploring vs exploiting) | `PARTIAL` | `predicted_sd` is computed and returned — but in the objective's own units with **nothing to compare it to**. ±3 is an exploit on a 40-point spread and an excursion on a 4-point one, and neither the return nor the skill says to make that comparison |
...
| 3.5 | has this optimization plateaued | `UNREPRESENTABLE` | nothing computes convergence. `campaign.optimize` runs exactly `n_rounds`; the only early stop is `space_exhausted`, which is discrete-space exhaustion. Probe `op-13` was graded *fabricated* for asserting "the last 1–2% gains are real" against a ±2% reproducibility **the user had stated in the question** |
...
| 3.6 | how much effort did optimization save versus screening | `PARTIAL` | no cost or labour field exists on any record — but `discrete_candidate_count` already sizes the full grid, so "best point in 11 proposals against a 96-cell grid" is computable today and is not computed |

...
model reads them to decide when to load a skill), and the optional `tools`/`mcp_servers`
declarations are checked against the live registries by `scripts.validate_skills` — the tool
registry (`agents.tool_registry`) and `settings.mcp_servers`. That check is the point: it turns
"this skill teaches a tool that no longer exists" from a silent stale-prose problem into a CI
failure. `tags` is free-form grouping for humans (and the eventual profile authoring in Stage 3).
...
class SkillManifest(BaseModel):
    ...
    # Capability dependencies this skill's judgment is written about. Optional: a skill that is
    # pure process guidance depends on nothing. Validated against the live registries, so a
    # renamed or deleted capability surfaces here rather than as stale prose in the skill body.

...
and an `ADD CONSTRAINT` that neither a `DROP CONSTRAINT IF EXISTS` precedes nor a `pg_constraint`
probe guards (the form `108` uses, which a replay leaves validated), since it aborts a *replay*
...
| `plan_approvals` | 020 (+034, 095) | `agent/plan_approval_store.py` | refused: kept through erasure (consumed rows are marked, not removed) |
| `behaviour_proposals` | 104 | `agent/behaviour_proposals.py` | refused: kept through erasure, for `plan_approvals`' reason one layer up — who decided what the agent was allowed to become. Retains the proposed document as well as the verdict, because a rejection is only evidence if the text it rejected is still there |
...
| `pending_requests` | 076, 079, 101 `premise_note_ids` | `durable/pending_store.py` | **refused**: the attribution for an answer that released a durable workflow, retained for the reason `plan_approvals` is — a settled row names who asked somebody to run, review or deliver something and who answered. Bounded by how often a *person* is asked something, which is human-paced and orders below the session tables; this is not `session_events`, where one turn writes many rows |
| `pending_request_answers` | 096 | `durable/pending_store.py` | **refused**: `pending_requests`' own attribution, archived when the question is asked again so the reopen cannot blank it — the same record one hop later, so the same refusal and the same erasure columns. INSERT only: an answer moved aside must not be editable. Bounded by how often a person is asked the *same* question twice, which is human-paced twice over |
...
| `reaction_records` | 052 (+053, 056, 066 `retracted_at`, 068, 081 `reaction_id`, 097) | `ingest/eln/records.py` | **nothing bounds it, deliberately** — one row per ELN entry (~1 kB), upserted by id, so the corpus tracks the source system and an amendment overwrites rather than appends. A row is the *only* readable form of a run (D-2026-08-25), so pruning one deletes a result; a deployment mirroring a 3M-entry ELN should expect a few GB and no growth beyond what the ELN itself holds. `retracted_at` (066, reserved by 068 and **read since 097**) carries a source-reported withdrawal: the tier that would have written it was deleted on review because the readers did not honour it, and `D-2026-09-13-a-withdrawal-is-a-fact-a-source-reports` rebuilt it from those readers — the producer is a field on the exported entry, and `is_current`, `eligible()`, the unfiltered retrieval sweep and `similar_reactions` all drop a withdrawn run while `read()` and `expand_note` still serve it saying so **081 restores the bare-`reaction_id` index 056 dropped**: 052 made that column the primary key and said "the PK carries the lookup", 056 widened the key to `(ingest_source, reaction_id)` and a composite B-tree cannot answer a predicate on its second column, so every turn-time read by bare id became a full scan — measured at 500,000 rows, 38.9 ms/3,955 buffers for one `read()` and a 76.3 ms parallel seq scan for `eligible()` at the shipped depth, against 0.10 ms and 1.10 ms with the index |
| `composed_workflows` | 100 (+103 the approval columns) | `templates/composed.py` | **nothing prunes it, deliberately** — one row per working procedure a chemist has asked the agent to write down, keyed `(owner, name)` so re-composing revises rather than adds. `document` is a whole `Template` as JSONB, which for a procedure of a handful of steps is a few kilobytes. Growth is bounded by `composed.MAX_PER_OWNER` (50) times the number of people, checked in `compose_workflow` before the write — the only table here whose bound is enforced by the application rather than by a sweep, because the row is a thing somebody is still using and a clock is the wrong owner of that decision. It *does* hold DELETE, unlike the refused-disposal tables around it, and **two things use it, neither of them a clock**: the owner saying so (`DELETE /workflows/{name}`, `/forget-workflow` — the other half of the cap, since re-composing over a name was otherwise the only way to make room and leaves a row whose name lies about its contents) and offboarding (`agent/leaver.py` erases a departing person's workflows, because a procedure names no result and cites no evidence and so belongs to their conversation rather than to the record). **103 adds `approved_fingerprint`/`approved_by`/`approved_at`**, which is what lets a composed workflow launch a durable job at all (`D-2026-09-15-an-approval-is-for-one-version-of-one-workflow`): the approval is keyed on the document's own hash, so re-composing lapses it with nothing having to clear it, and the three columns are written by one front-door route and by nothing the model can call |
...
| `experiment_arm_results` | 107 (+108 `value` finite `CHECK`) | `protocols/result_store.py` | **nothing prunes it, and INSERT-only by grant** — one row per measured outcome per arm, so a fully-read 384-well plate measuring yield and purity is 768 rows of a dozen scalars, which is small beside the design document it points at. Append-only for the reason the revisions above are, arriving from the other side: a re-measured well is a *second observation* rather than a correction, so a credential that could UPDATE one could make two disagreeing assays look like one agreeing measurement, and a sweep that pruned one would delete the evidence that they disagree (`D-2026-09-21-an-outcome-is-a-third-table-not-a-column-on-either-tier`). It cascades from `experiment_protocols`, which is itself never deleted, so the cascade is unreachable |

...
| `corpus_reactions` | 069 | `ingest/labels/corpus.py` | derived and rebuildable: refilled by re-draining the corpus |
| `corpus_cursors` | 070 | `ingest/labels/cursor.py` | — (one row per append-only corpus source; deleting a row is the supported way to force a full re-walk) |

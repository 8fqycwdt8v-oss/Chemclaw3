...
| `turn_costs` | 033 (+060 `outcome`/`error_code`/`model`/counts/`ttft_seconds`) | `agent/turn_cost_store.py` | — |
...
| `reaction_records` | 052 (+053, 056, 066 `retracted_at`) | `ingest/eln/records.py` | **nothing bounds it, deliberately** — one row per ELN entry (~1 kB), upserted by id, so the corpus tracks the source system and an amendment overwrites rather than appends. A row is the *only* readable form of a run (D-2026-08-25), so pruning one deletes a result; a deployment mirroring a 3M-entry ELN should expect a few GB and no growth beyond what the ELN itself holds. A row the source has *retracted* (066) is kept for the same reason and simply stops being served as current evidence |

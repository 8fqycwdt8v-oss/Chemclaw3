...
| `corpus_reactions` | 062 | `ingest/labels/corpus.py` | derived and rebuildable: refilled by re-draining the corpus |
| `corpus_cursors` | 063 | `ingest/labels/cursor.py` | — (one row per append-only corpus source; deleting a row is the supported way to force a full re-walk) |

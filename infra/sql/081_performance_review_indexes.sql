-- Two independently measured index gaps from the 2026-09-04 performance review, batched into one
-- migration for the reason `046` batches three: each is a single small DDL statement, none depends
-- on the other, and both were found by the same review of the same question ("what does this
-- schema do at 200 concurrent chemists").
--
-- `reaction_records_reaction_id_idx` — `ingest/eln/records.py`'s `_SELECT_ONE` and `_SELECT_KNOWN`
-- both filter on the bare `reaction_id`, and **no index has ever led with that column**: the
-- primary key is `(ingest_source, reaction_id)` (`056`), `reaction_records_filter_idx` is
-- `(project, performed_at)` (`052`) and `reaction_records_retracted_idx` is partial over the
-- retracted rows (`066`). `_SELECT_BODIES` gets it right — it filters `ingest_source` too — which
-- is what made the other two look deliberate when they were not. Both are on a live path:
-- `agent/graph_tools.py::expand_note` resolves a `reaction-<id>` citation through `read()`, and
-- `kg/validate.py` plus the warehouse retriever ask `known()` for a page of ids at a time.
--
-- Measured against this schema on 500 000 records (240 MB), PostgreSQL 16.15:
--
--   _SELECT_ONE,   one id  : Index Scan on the pkey with the Index Cond on its *non-leading*
--                            column -> 2 467 buffers, 24.1 ms   ->  4 buffers, 0.045 ms  (535x)
--   _SELECT_KNOWN, 50 ids  : Parallel Seq Scan -> 27 778 buffers (217 MB), 152.3 ms
--                                              -> Index Only Scan, 203 buffers, 0.80 ms  (190x)
--   _SELECT_BODIES, 50 ids : unchanged (0.55 ms) — it was already served by the primary key.
--
-- Cost: 15 MB of index against a 240 MB table, and one more index to maintain on an upsert that
-- already maintains three. It is `(reaction_id)` alone rather than `(reaction_id, ingest_source)`
-- because the pair is what the primary key already answers, and the bare column is what lets
-- `_SELECT_KNOWN` — which projects nothing else — take an Index Only Scan.
CREATE INDEX IF NOT EXISTS reaction_records_reaction_id_idx

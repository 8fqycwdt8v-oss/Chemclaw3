...
def test_every_declared_job_that_takes_a_solvent_declares_the_precondition() -> None:
    """The rule is only worth anything on the jobs that can violate it — all ten of them.
    ...
    `rank_species_across_solvents` — which takes `solvents`, plural, and came through this
    assertion to get here. A sweep that adapted silently would let the eleventh arrive with no
    precondition and fail thirty seconds into a durable run with tblite's own "String value for
    epsilon was not found among database of solvents".
    """

...
async def test_inmemory_dense_ranks_by_cosine() -> None:
...
async def test_inmemory_lexical_ranks_by_term_overlap() -> None:
...
async def test_reindex_then_dense_search_finds_the_semantic_note(tmp_path: Path) -> None:
...
async def test_reindex_against_postgres_heals_a_model_swap_without_full(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
async def test_postgres_note_index_round_trip() -> None:
    ...
    await index.upsert(
        [NoteRecord(note_id="note-001", text="amide coupling epimerization", embedding=embedding)],
        note_embedding_key(),
    )
...
async def test_postgres_fingerprints_round_trip_and_omit_unset_rows() -> None:
...
async def test_reindex_incremental_against_postgres_embeds_only_the_change(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:

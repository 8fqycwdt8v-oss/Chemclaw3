...
into `data/` (D-155). Each pass ended with a map written by hand, and `ARCHITECTURE.md` closes by
...
1. **Every directory has a `README.md`.** GitHub renders one the moment you click a folder, which
   ...
   fourteen packages before D-155.
...
def test_the_map_lists_nothing_that_has_gone() -> None:
    ...
    This is the half a restructure breaks. `src/chemclaw/mcp/` was dissolved in D-155 and its row

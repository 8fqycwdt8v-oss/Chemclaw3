...
async def test_read_campaign_miss_returns_none() -> None:
...
async def test_upsert_then_read_round_trips_every_field() -> None:
...
async def test_a_second_upsert_keeps_the_original_opener() -> None:
...
async def test_record_is_append_only_and_returns_increasing_ids() -> None:
    ...
    first_id, _ = await store.record(_campaign(campaign_id), Suggestion(campaign_id=campaign_id))
    second_id, _ = await store.record(_campaign(campaign_id), Suggestion(campaign_id=campaign_id))

    assert first_id != second_id
    assert len(await store.suggestions_for(campaign_id, 10)) == 2


async def test_a_suggestion_round_trips_its_candidates_observations_and_provenance() -> None:
...
async def test_suggestions_for_orders_newest_first_and_honours_the_limit() -> None:
    ...
    for round_index in range(5):
        await store.record(
            ...
            Suggestion(
                campaign_id=campaign_id,
                observations=[Observation(params={"temperature": float(round_index)}, value=1.0)],
            ),
        )
...
async def test_distinct_campaigns_keep_independent_suggestion_histories() -> None:
...
async def test_the_inline_path_keeps_appending_because_its_run_id_is_empty() -> None:
...
async def test_a_suggestion_round_trips_the_space_it_was_proposed_against() -> None:

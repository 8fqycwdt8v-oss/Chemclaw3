...
async def test_structuring_a_request_stores_revision_one_as_a_request(
...
async def test_a_structured_request_returns_json_the_front_end_can_parse(
    store: InMemoryDesignStore,
) -> None:
...
async def test_a_salt_is_how_a_second_design_for_one_ask_is_opened(
    store: InMemoryDesignStore,
) -> None:
    first = await _open()
...
async def test_structuring_refuses_before_it_stores_anything(
    store: InMemoryDesignStore,
) -> None:
...
async def test_drafting_refuses_a_design_that_cites_nothing(
    store: InMemoryDesignStore,
) -> None:
...
async def test_drafting_accepts_a_design_that_cites_a_precedent_and_a_tool(
    store: InMemoryDesignStore,
) -> None:
    opened = await _open()
...
async def test_a_revision_reports_the_paths_it_changed(
    store: InMemoryDesignStore,
) -> None:
...
async def test_a_revision_derived_from_a_stale_head_is_refused(
    store: InMemoryDesignStore,
) -> None:
    ...
    with pytest.raises(ChemclawError, match="is at revision 3"):
        await _draft(opened.design_id, 2, arms=3, change_note="added a third arm from a stale read")


async def test_a_plate_format_computes_the_layout(store: InMemoryDesignStore) -> None:
...
async def test_a_randomized_plate_records_the_seed_that_reproduces_it(
    store: InMemoryDesignStore,
) -> None:
    opened = await _open()
    receipt = ProtocolReceipt.model_validate_json(
...
async def test_a_plate_that_cannot_hold_the_arms_names_the_smallest_that_can(
    store: InMemoryDesignStore,
) -> None:
...
async def test_an_unknown_plate_format_is_refused_before_anything_is_stored(
    store: InMemoryDesignStore,
) -> None:
    opened = await _open()
...
async def test_drafting_after_structuring_the_same_ask_stores_the_next_revision(
    ...
    drafted = ProtocolReceipt.model_validate_json(await _draft(intake.design_id, intake.revision))
...
async def test_restructuring_the_ask_keeps_the_protocol_it_was_drafted_into(
    ...
    again = ProtocolReceipt.model_validate_json(await tools.structure_experiment_request(corrected))
...
async def test_restructuring_grades_the_checks_at_the_stage_the_design_is_at(
    store: InMemoryDesignStore,
) -> None:
...
async def test_passing_a_plate_format_on_a_revision_lays_the_plate_out_again(
    store: InMemoryDesignStore,
) -> None:
...
async def test_reading_an_unknown_design_is_refused_with_a_next_step(
    store: InMemoryDesignStore,
) -> None:
...
async def test_reading_an_unknown_revision_names_the_revision(
    store: InMemoryDesignStore,
) -> None:
    opened = await _open()
    await _draft(opened.design_id, opened.revision)
...
async def test_reading_a_plate_whose_outcome_mixes_units_keeps_the_rest_of_the_readout(
    ...
    opened = await _open()
    await _draft(opened.design_id, opened.revision, arms=3)
    ...
    await results.append(
        ...
        2,
        [
            ArmResult(arm_id="A1", outcome="yield", value=85.0, unit="%"),
            ArmResult(arm_id="A2", outcome="yield", value=0.9, unit="fraction"),
        ],
        author_kind="agent",
    )

    readout = tools.PlateReadout.model_validate_json(
        await tools.read_plate_results(opened.design_id, outcome="yield")
    )

    assert readout.observations == []
    assert "more than one unit" in readout.observations_refused
    assert "'A1'" in readout.observations_refused and "'A2'" in readout.observations_refused
    assert readout.outcomes.arms_without_results == ["A3"]
    assert len(readout.outcomes.results) == 2
...
async def test_a_read_selects_the_revision_it_is_asked_for(
    store: InMemoryDesignStore,
) -> None:
    opened = await _open()
    await _draft(opened.design_id, opened.revision)
...
async def test_finding_rejects_a_status_that_is_not_a_status(
    store: InMemoryDesignStore,
) -> None:
...
async def test_finding_lists_what_was_stored_and_filters_by_status(
    store: InMemoryDesignStore,
) -> None:
...
async def test_finding_filters_by_project_and_returns_parseable_json(
    store: InMemoryDesignStore,
) -> None:
...
async def test_an_empty_listing_is_valid_json_rather_than_nothing(
    store: InMemoryDesignStore,
) -> None:
...
async def test_an_inferred_ask_needs_no_chemist_message(store: InMemoryDesignStore) -> None:

...
def fixture_bundle(monkeypatch: pytest.MonkeyPatch) -> Iterator[str]:
    ...
    resolving (D-165), which is the behaviour under test rather than an obstacle to it. The fixture
...
# --- DARK-2: the step is authorized and audited as its requester (D-165) -----------------------
...
def test_every_template_step_activity_is_registered_on_a_worker() -> None:
    ...
    run_tool_step ... is not registered on this worker". Found by running it live for D-165.

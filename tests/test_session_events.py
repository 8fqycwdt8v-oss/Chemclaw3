...
async def test_record_and_claim_round_trip() -> None:
...
async def test_concurrent_claims_never_double_deliver() -> None:
    ...
    first, second = await asyncio.gather(claim_unconsumed(session_id), claim_unconsumed(session_id))
...
async def test_duplicate_dedupe_key_inserts_once() -> None:
    ...
    await record_session_event(session_id, "job_completed", {"job_id": "j-2"}, dedupe_key=other_key)
    claimed = await claim_unconsumed(session_id)
    assert [e.payload["job_id"] for e in claimed] == ["j-1", "j-2"]  # the retry deduped


async def test_null_dedupe_key_keeps_plain_append() -> None:

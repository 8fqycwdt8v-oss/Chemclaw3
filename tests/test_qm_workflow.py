...
def test_the_actor_reaches_the_hpc_side_from_the_run_memo() -> None:
    ...
    the D-156 cache instead of the cluster path it is named for — and against a real Postgres it
...
def test_the_finished_result_is_persisted_and_cited(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    The durability half of D-156. Before it, the only homes for an hours-long cluster result were
...
def test_an_identical_second_run_is_served_from_the_store(
    ...
    """The reuse half of D-156 — the part that actually stops paying twice for cluster time.

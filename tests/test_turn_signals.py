...
def test_an_approval_signal_carries_the_holds_handle_to_the_stream() -> None:
    ...
    approval arrived renderable but unanswerable — mutation-verified by dropping the
    `record_approval_request` call, which puts `approval_id` back to "".
    """
...
def test_a_plan_approval_still_has_no_handle() -> None:
    ...
    It is answered by the next turn, not by a decision endpoint, and an id there would point a
    surface at a hold that does not exist.
    """

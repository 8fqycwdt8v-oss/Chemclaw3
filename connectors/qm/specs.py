...
`connectors/calc/specs.py`: pydantic, `chemclaw.config`, `chemclaw.chem` and `chemclaw.ids` only,
...
def qm_job_key(spec: QmJobSpec) -> str:
    ...
    `chemclaw.ids.stable_hash` (SHA-256) with every other identity key in the system.

...
class CategoricalParameter(BaseModel):
    ...
    informs its neighbours (U1). `bo.featurize` fills `descriptors` from `structures`.
...
class ScreeningDesign(BaseModel):
    """A full-factorial screening design: one row per combination of categorical levels (D-092).

    Distinct from `Candidate`/BO's one-batch-at-a-time proposals: this is the complete Cartesian
    product a classical categorical screen runs up front, generated by `bo.engine.factorial_design`.
    """

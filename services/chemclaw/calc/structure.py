...
- `multiplicity` generalizes the previous hard closed-shell rejection into a
  ...
  (`calc.xtb_props`) a legitimate open-shell calculation rather than a silent one.

Coordinates are in **Angstrom** — the interchange unit of RDKit, XYZ files, and this
whole layer; `calc.xtb_engine` is the single boundary that converts to atomic units.
"""
...
from calc.xtb_engine import geometry, parse_molecule
...
def structure_from_mol(
    ...
    *,
    ...
    Expects explicit hydrogens (`calc.xtb_engine.parse_molecule` output) so the

...
rather than an index is that the parsed notes are *already* cached in memory by `kg.graph`
...
def notes_for_calculation(notes_dir: Path, calc_key: str) -> list[Note]:
    ...
    Reads through `kg.graph.load_notes`, so it shares the parsed-note cache with every other reader

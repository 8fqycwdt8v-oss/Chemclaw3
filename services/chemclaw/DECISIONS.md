...
## D-092 — The connector seam: one way to add a tool, a skill, or an agentic workflow
...
## D-093 — Stage C: the domain connectors, and two defects the migration surfaced

`safety`, `chem` and `calc` moved out of the agent's process to their own bundles (D-092's seam).
...
2. **`AgentProfile.tool_names` could no longer reach a migrated tool.** Profiles had two dials —
   ...
   connectors are per-turn objects (D-092) — on a shared connector it would have been a cross-turn
...
## D-094 — `bo` as the reference connector-owned durable capability
...
strings from `connector.yaml` — the property D-092 claimed and this is the first exercise of. The
...
## D-095 — Stages D and E: profiles select an agent, templates fix a procedure

The connector seam (D-092) made *capability* one thing to add. These two stages do the same for the

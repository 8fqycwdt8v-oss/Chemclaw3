...
Source: every open (`- [ ]`) item in `BACKLOG.md`, plus the open rows of `DEFERRED.md` that the
...
### Config extensibility (`docs/audit/10-config-extensibility.md`)
...
### Capability gaps (`docs/research-review.md`, `docs/parity-plan.md`)
...
| 21 | Gate-until-trigger parity items (OCR/vision, Veeva/SAP/LIMS connectors, GAMP-5 artifacts, multi-agent mesh) | DEFER | Each has a written trigger in `docs/parity-plan.md`; none holds. The mesh in particular stays declined on KISS grounds (one agent + skills + Temporal fan-out covers it, D-030/F10-D). |
...
| 35 | AG-13 agent-behavior/prompt/skill regression eval · F10-B3 report-prose faithfulness · audit-chain tip-truncation anchor · live-retriever drift | BLOCKED / DEFER | Each is documented in `DEFERRED.md` with a trigger that is a live LLM, an in-workflow prose step, a regulator requirement, or a populated deployment graph. Re-checked; unchanged. |
...
*Design.* `docs/runbook.md`: a short "exposed deployment" subsection with both env-var facts and the
...
`docs/runbook.md` how to read the backlog of alerts (`session_events` filtered to the channel) and
...
*Design.* `docs/workflow-versioning.md`: what counts as a logic change (control flow, activity call
...
check to the deploy checklist in `deploy/README.md` and cross-link from `docs/runbook.md`.
...
- ADRs: **D-077** (event emission), **D-078** (memory supersede), **D-079** (workflow versioning),
  **D-080** (safety screening, if C2 is approved). Wave A items are too small for ADRs and are
  recorded in `BACKLOG.md` on completion.
- `BACKLOG.md` and `DEFERRED.md` updated at the end of each wave; the DROP verdicts (items 3, 23,
  25-stand-in, 27, 28, 32) are corrected in `BACKLOG.md` as part of this change, since they are

...
(`workflows.template_job`), where anything non-deterministic breaks replay. Given the same inputs
...
least afford — and `templates.manifest` already rejects references that cannot resolve, so reaching

...
Settings(ObservabilitySettings, TemporalSettings, ...)`). Each mixin holds its section's fields,
validators, and derived properties, so a reader finds everything about one concern in one place
— while the composed class keeps every attribute flat (`settings.postgres_dsn`) and every env
name unprefixed-by-section (`CHEMCLAW_POSTGRES_DSN`), exactly as before the split. A cross-field
validator lives in the section that owns the relationship.
...
class CalculatorSettings(BaseSettings):
    ...
    # GFN2-xTB). `xtb_embed_seed` fixes RDKit 3D embedding so results are reproducible; it is
    # part of the cache key so changing it recomputes.
...
class Settings(
    ...
    Composed from the per-domain section mixins above; every field stays a flat attribute with
    its original env name, and this `model_config` (prefix,

...
which is precisely why `agents.chemclaw_agent.connector_tools` builds fresh tools per turn
...
async def stamp_turn_identity(request: httpx.Request) -> None:
    ...
    Registered on the connector's own client (`connectors.registry`), so it runs inside the task

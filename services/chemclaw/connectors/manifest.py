...
class NoAuth(BaseModel):
    ...
    Correct for a stdio connector (a subprocess of our own pod, under our own identity) and for a
    loopback HTTP connector in dev. `ConnectorManifest` refuses it for a non-loopback URL unless the
    deployment has explicitly opted into insecure binding, reusing the front door's loopback rule
    rather than inventing a second notion of "safe address".
    """
...
class BearerAuth(BaseModel):
    ...
    `token_env` names the variable rather than carrying the token, so no credential is ever written
    to a manifest in the repo. It is read per request, not at import, so a rotated secret is picked
    up without a restart — and a missing variable fails the call with a clear error instead of
...
# How chemclaw authenticates to a connector, discriminated on `mode`. Only the two modes with a real
# caller are built: `none` for the trust-boundary cases and `bearer` for everything in-cluster. The
# Entra service-identity and on-behalf-of modes are a documented extension point, not a stub — OBO
# needs the user's raw access token, which `service.auth.Principal` deliberately does not carry, and
# neither exchange can be verified without a real tenant (see `docs/connector-plan.md` §11).
...
class HttpEndpoint(BaseModel):
    ...
    `/healthz`, while a third-party MCP server may expose nothing, and reporting such a connector as
    "unprobed" is honest where guessing a path would produce a false alarm. `request_timeout` (whole
    seconds, as MAF types it) keeps an unreachable host from hanging a turn; `None` defers to MAF's
...
class StdioEndpoint(BaseModel):
    ...
    The same agent-facing surface as `HttpEndpoint` — callers never branch on the transport — but no
    identity headers travel: there is no request to attach them to, and the subprocess already runs
    under our identity. Kept because it is the zero-infrastructure path for a local capability and
...
class JobSpec(BaseModel):
    ...
    reaches it without importing it. `workflow` is a Temporal workflow **type name** (a string) and
    `task_queue` is where its worker polls, so core has no build-time dependency on the connector at
    all — and moving a workflow from core's worker to the connector's own is a one-line change here
    (`docs/connector-plan.md` §5.3).

    `expensive` marks the job for the coarse `authorize_trigger` gate (a costly HPC/BO run must be
    entitled, not merely authenticated). `publish_to_graph` lets core PR-gate a `Note` the job's
    result carries — the write still goes through `kg.pr_gate`, never through the connector.
    """

    model_config = ConfigDict(extra="forbid", frozen=True)

    # The advertised tool name, so it is keyed and gated exactly like a hand-written tool
    # (`tool_role_gates`, `DEFAULT_WRITE_TOOL_GATES`, profile narrowing all address this string).
    name: str = Field(min_length=1, pattern=r"^[a-z][a-z0-9_]*$")
    workflow: str = Field(min_length=1)
    task_queue: str = Field(min_length=1)
    # The first line of the generated tool's docstring — what the model reads when deciding to call
    # it. `description` carries the rest (when to use it, what the id is for, idempotency).
    summary: str = Field(min_length=1)
    description: str = ""
    # A job declares its launch arguments one of two ways, and the choice is about *fidelity*, not
    # taste. `params` is the easy path: flat, closed-type arguments declared inline, which covers a
    # job whose input is a handful of scalars (a SMILES, a method name, a count). `params_model` is
    # the full-fidelity path: a dotted `module:Attribute` reference to an existing pydantic model,
    # for a job whose input is a rich domain object — a nested optimization problem with
    # discriminated feature kinds cannot be re-declared in YAML without losing exactly the structure
    # that makes the model call it correctly, and re-declaring it would be a second source of truth
    # for a schema that already exists in code. Declaring neither is a job with no arguments.
...
class ConnectorManifest(BaseModel):
    ...
    A connector must contribute *something reachable* — an endpoint, a job, or both — for the same
    reason `sources.base.SourceSpec` rejects a source with neither half: a connector that serves no
    tools and runs no jobs is not a connector. A bundle that only ships skills or profiles belongs
    in the skills tree, not here.

    The endpoint's `tools` allow-list is **read/compute only** by contract. Mutation goes through a
    `jobs:` entry (which core authorizes, dry-run-gates and attributes) or stays a core PR-gate
    tool.
    That is the existing `allowed_tools` boundary (D-029) promoted from a convention to a validated
    contract: `make connector-validate` refuses a name matching the mutating-tool prefixes, so a
    connector cannot quietly hand the model a write path.
    """
    ...
    # Names of the `SKILL.md` folders under this bundle's `skills/` dir and the profile files under
    # its `profiles/` dir. Declared rather than inferred so a stray folder is a CI failure instead
    # of a silently-shipped skill (`scripts.validate_connectors`).

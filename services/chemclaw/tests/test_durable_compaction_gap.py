...
`agents.chemclaw_agent._build_compaction` passes an `after_strategy`, and its docstring said it
...
def test_compaction_after_run_is_a_no_op_without_that_state() -> None:
    ...
    async def _drive() -> None:
        ...
        assert called == [], (
            ...
            "session state after all — re-check `agents/session_store.py` before trusting its note"
        )
...
def test_the_load_repair_writes_back_which_is_why_a_limit_is_unsafe() -> None:
    ...
    """
    source = (Path(__file__).resolve().parents[1] / "agents" / "session_store.py").read_text()

...
Since the xTB capability layer (plan X1) this module is thin: `chemclaw.science.calc.structure` owns
the geometry and its validation, `chemclaw.science.calc.xtb_spec` owns the cache key, and
`chemclaw.science.calc.xtb_engine` owns the SCF. What is left here is the one thing specific to a
...
from chemclaw.science.calc.xtb_engine import gfn2_energy
...
def _sp_structure(smiles: str, charge: int) -> Structure:
    ...
    This is the policy `chemclaw.science.calc.pka` and `chemclaw.science.calc.xtb_props` already
    apply; the energy path
...
def run_xtb(job: XtbInput) -> XtbResult:
    ...
    `chemclaw.science.calc.structure.Structure`, so every xTB task inherits them identically.
    """

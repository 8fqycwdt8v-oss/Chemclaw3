...
Used by every xTB-based calculator (`chemclaw.science.calc.xtb`, `chemclaw.science.calc.pka`,
`chemclaw.science.calc.xtb_props`) so the
...
interchange unit of RDKit, XYZ files, and `chemclaw.science.calc.structure.Structure`), and the
...
def require_closed_shell(mol: Chem.Mol, charge: int) -> None:
    ...
    Kept for `chemclaw.science.calc.pka`, whose v1 calibration is defined over neutral closed-shell
...
def conformer_positions(mol: Chem.Mol, conf_id: int = -1) -> tuple[np.ndarray, np.ndarray]:
    ...
    this branch made `chemclaw.science.calc.xtb_engine` the **single unit boundary** (X1):
    everything above it
...
def evaluate_point(calc: Calculator, positions: np.ndarray) -> tuple[float, np.ndarray, np.ndarray]:
    ...
    `chemclaw.science.calc.xtb_props` makes for charges and bond orders).

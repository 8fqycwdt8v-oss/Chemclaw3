...
for a Hessian (`chemclaw.science.calc.xtb_thermo`), a torsion profile
(`chemclaw.science.calc.xtb_scan`), and a reaction
free energy (`chemclaw.science.calc.reaction`).
...
class OptimizationSummary(BaseModel):
    ...
    geometry keeps flowing in-process (`chemclaw.science.calc.reaction` consumes
    `OptimizationResult`
...
def optimize_structure(spec: OptSpec, structure: Structure) -> OptimizationResult:
    ...
    L-BFGS below it — measured, see `chemclaw.science.calc.xtb_cli`. The in-process path remains the
...
def _optimize_with_library(spec: OptSpec, structure: Structure) -> OptimizationResult:
    ...
    (`chemclaw.science.calc.anc`, plan X9).
...
def _optimize_with_binary(spec: OptSpec, structure: Structure) -> OptimizationResult:
    ...
    exactly the input surface `chemclaw.science.calc.xtb_cli` refuses to have.
...
def _force_field_result(
    ...
    `chemclaw.science.calc.xtb_cli` documents as unreliable. Without it there is no evidence at
    all, and
...
async def run_cached_optimization(
    ...
    `chemclaw.science.calc.reaction` optimizes each species it was handed, and
    `chemclaw.science.calc.xtb_thermo` needs
    ...
    (`chemclaw.science.calc.geometry`, STO-4), so a later caller asking for "a good geometry of
    this compound" can

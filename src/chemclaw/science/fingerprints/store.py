...
class FingerprintSearch(BaseModel, Generic[HitT]):
    ...
    def verdict(self) -> str:
        ...
        `Chemclaw3-mcp:servers/safety/src/chemclaw_mcp_safety/engine/screen.py` — which carried its "this
        is not a safety assessment" disclaimer as a bare property for exactly this reason and had
        **zero** production callers, until a live run showed a chemist being told "no hazards
        detected" six times. The tool docstring is read once when the tool is defined; the result
        payload is what sits in the context window when the answer is written.

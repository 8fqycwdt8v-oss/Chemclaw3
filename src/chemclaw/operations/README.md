...
ran, what the agent wrote into the knowledge graph, and where the turns and tokens went.
...
1. **Counts and identifiers only — never a caller's free text.** `audit_events.arguments`,
   `audit_events.detail` and `job_records.rationale` all hold text a caller supplied, and there
...
3. **A row this system never wrote is never inferred.** `authorship` reports the notes the agent
   wrote and how those calls ended. It carries a `boundary` sentence saying it is not a share of a
   document's authorship, because a human edit made in the git host leaves no row here.
...
asserted, nothing reaches the knowledge graph, and nothing is remembered.

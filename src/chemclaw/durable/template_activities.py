...
class ResolvedJob(BaseModel):
    ...
    `payload` is here because resolution and authorization are one act, not two (D-165). The
...
async def authorize_job_step(step: JobStepInput) -> ResolvedJob:
    ...
    **The template's job step used to do none of this** (DARK-2, D-165). `ResolvedJob` carried the
...
# D-165; it is the same class as the eight defects D-155 collected, where a feature is written,
    ...
    for connector in connectors:
        for function in connector.functions:
            if function.name == step.tool:
                # Through the *same* governed path as the in-process branch above (D-165). This
...
async def _call_function_tool(tool: Any, step: ToolStepInput) -> Any:
    ...
    Used for both halves of the surface since D-165. The connector half used to call
...
async def run_agent_step(step: AgentStepInput) -> str:
    ...
    **Run without the harness, whatever the deployment's default is** (D-165). Two reasons, and the

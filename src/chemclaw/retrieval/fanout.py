...
async def _sweep(state: BranchState, config: RunnableConfig) -> dict[str, Any]:
    ...
    try:
        ...
        # through it is invisible to `chemclaw_degradations_total` and to `tests/test_degraded.py`,
...
def _report(name: str, found: int, *, failed: bool = False) -> None:
    ...
    if writer is None:  # no graph runtime, or nothing consuming a custom stream
        ...
        # nothing *and* for a source whose database was unreachable — byte-identical. The module
        # docstring above claims the branch exists to tell "returned nothing" from "was not asked";
        # it could not tell either from "raised" until this field existed.

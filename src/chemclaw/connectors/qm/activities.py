...
async def lookup_qm_result(job: QMJobInput) -> QmCacheLookup:
    """Return this calculation's store key, and its result if it has already been computed (D-154).
...
async def persist_qm_result(result: QMJobResult) -> str:
    """Persist the finished calculation in the shared calculation store; return its key (D-154).

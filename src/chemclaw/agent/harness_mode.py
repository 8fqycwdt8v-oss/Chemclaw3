...
async def current_plan_hash(session: AgentSession) -> str:
    ...
    **This used to hash the rendered lines, and that made the gate unenforceable** (D-164). The
...
def consume_plan(session: AgentSession, plan_hash: str) -> None:
    ...
    **This is what makes an approval authorize a request rather than a session** (D-164), and it
...
def revoke_execute(session: AgentSession) -> str:
    """Return the session to plan mode — the mirror `grant_execute` never had (D-164).

...
`api/runner.py`; this module only scores. The durable hold itself exists — `InteractionApprovalWorkflow`
(D-032) — but nothing routes a `review_required` answer into it, so today a low-confidence answer is
marked, not blocked. That wiring is the deferred part; see docs/planning/DEFERRED.md.
"""

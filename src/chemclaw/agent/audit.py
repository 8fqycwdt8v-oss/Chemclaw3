...
def metric_tool_name(request: Any) -> str:
    ...
    **The model's string is not a metric label**, which is why this takes the request rather than a
    ...
    It therefore takes no `name` argument: it had one, every caller passed
    `request.tool_call["name"]`, and the body never read it — a parameter whose value is
    exactly the string this function exists to refuse reads like the clamp is a comparison,
    and it is not.

...
_INSTRUCTIONS = (
    ...
    "hit's job id to get_durable_job_status for that run's full result. find_calculations asks "
    "that same question per molecule — what is already known about this structure — and nothing "
    "prunes that store, so ask it before committing compute. (2) For cross-learning by "

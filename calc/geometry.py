...
it ran. That is precisely the cache dishonesty `calc.xtb_spec` was written to prevent: a key must
...
async def record_optimization(
    ...
    *,
    ...
    `calc.xtb_opt` — the optimizer calls *this*, and the dependency has to point one way. It also
